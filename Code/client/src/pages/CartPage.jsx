...
export default function CartPage() {
  ...
  const fetchCart = useCallback(async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Couldn't load your cart.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handleQuantityChange(productId, quantity) {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Couldn't update quantity.");
    } finally {
      setUpdatingId(null);
    }
  }
  ...
  if (loading) return <div className="page-content"><LoadingState label="Loading your cart…" /></div>;
